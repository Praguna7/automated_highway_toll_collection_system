...
function Bill({ discountTitle, brandName, brandCategory, brandContactNo, brandWebsite, brandLogo, userName, userAddress, userContactNo, userCards, userImage, discountStartDate, discountEndDate, discountLocation, discountUrl, discountImage, noGutter, dataId }) {
  ...
  const [vehiclesData, setVehiclesData] = React.useState({ percentage: '' })
  ...
  const fetchDataById = async (dataId) => {
    ...
    try {
      const getData = await getDoc(doc(db, "vehicles", dataId));
      if (getData.exists()) {
        setDbDiscountData(getData.data())
        setDbDiscountBankData(getData.data().user)
        setDbDiscountBrandData(getData.data().brand)
        setLocationTableData(getData.data().locations)
        setDiscountTableData(getData.data().vehicles)
        setDiscountCardData(getData.data().vehicles)
        setCardDropdown(getData.data().user.cards)
      } else {
  ...
  const deleteById = async (dataId) => {
    ...
    try {
      const reference = doc(db, 'vehicles', dataId)
  ...
  const onUpdateVehiclesData = async (e) => {
    e.preventDefault()
    // put data into firestore
    const deleteData = {
      vehicles: arrayRemove(...discountTableData)
    }
    const updateData = {
      title: dbDiscountData.title,
      locations: locationTableData,
      vehicles: arrayUnion(...discountCardData.map((items) => {
        return items
      })),
    ...
    try {
      setLoading(true)
      if (dataId) {
        const delDocRef = doc(db, "vehicles", dataId)
        await setDoc(delDocRef, deleteData, { merge: true })
      }
      if (dataId) {
        const updDocRef = doc(db, "vehicles", dataId)
  ...
  const onSaveVehiclesOnCards = () => {
    if (vehiclesData.percentage === '' && selectedCard === '') {
      return null
    } else {
      ...
      setDiscountCardData([...discountCardData, {
        ...
        percentage: vehiclesData.percentage
      }])
      setSelectedCard('')
      setVehiclesData({ percentage: '' })
    }
  }
  const onDelVehiclesOnCardsList = (index) => {
          ...
          >
            ...
            <Paper elevation={5} sx={{ m: 2, borderRadius: '1rem', pb: 2 }}>
              ...
              <MDInput
                ...
                required
                value={vehiclesData.percentage}
                onChange={(e) => setVehiclesData({
                  ...vehiclesData,
                      ...
                      {discountCardData.map((items, index) => {
                        return (
                          ...
                          >
                            ...
                            <TableCell component="th" scope="row" align="right">
                              <Button onClick={() => onDelVehiclesOnCardsList(index)}><DeleteIcon color="error" /></Button>
                            </TableCell>
                          </TableRow>
                        )
                      })}
      ...
      >
        <MDBox width="100%" display="flex" flexDirection="column">
          ...
          <MDBox mb={0} lineHeight={0} display="flex" flexDirection="row" alignItems="center">
            ...
            <MDAvatar sx={{ cursor: "pointer" }} onClick={userImageModalOpen} src={userImage} size="sm" />
          </MDBox>
          <MDBox mb={0} lineHeight={0}>
            <MDTypography variant="caption" color="text">
              Bank Cards:&nbsp;&nbsp;&nbsp;
