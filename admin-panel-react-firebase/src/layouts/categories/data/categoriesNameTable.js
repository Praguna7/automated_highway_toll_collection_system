...
function Data() {
  ...
  const fetchAllCollectionsCategories = async () => {
    ...
    try {
      ...
      const query3 = query(collection(db, "vehicles"), where("brand.category", "==", categoryValue));
  ...
  const delAllCollectionsCategories = async () => {
    ...
    try {
      ...
      const query3 = query(collection(db, "vehicles"), where("brand.category", "==", categoryValueDel));
      const querySnapshotCarousel3 = await getDocs(query3);
      querySnapshotCarousel3.forEach((doc) => {
        setVehiclesId(doc.id)
      })
