...
function Bill({ name, contactNo, website, category, logo, noGutter, dataId }) {
  ...
  const [userLogoModal, setBankLogoModal] = React.useState(false);
  ...
  const fetchAllSalesBrandCarousels = async () => {
    ...
    try {
      ...
      const query2 = query(collection(db, "vehicles"), where("brand.uid", "==", dataId));
  ...
  const onUpdateBrand = async (e) => {
    ...
    try {
      ...
      if (vehiclesId) {
        const discountDocRef = doc(db, "vehicles", vehiclesId)
  ...
  const userLogoModalOpen = () => setBankLogoModal(true);
  const userLogoModalClose = () => setBankLogoModal(false);
  return (
    <>
      <MDSnackbar
        color="success"
      ...
      <Modal
        open={userLogoModal}
        onClose={userLogoModalClose}
        aria-labelledby="userLogoModal-userLogoModal-title"
        aria-describedby="userLogoModal-userLogoModal-description"
      >
        <Box sx={style}>
          <Card
            sx={{
      ...
      >
        <MDBox width="100%" display="flex" flexDirection="column">
          ...
          <MDBox mb={0} lineHeight={0} display="flex" flexDirection="row" alignItems="center">
            ...
            <MDAvatar sx={{ cursor: "pointer" }} onClick={userLogoModalOpen} src={logo} size="sm" />
          </MDBox>
        </MDBox>
      </MDBox>
    </>
