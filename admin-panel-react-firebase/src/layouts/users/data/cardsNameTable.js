...
function Data(dataId) {
  ...
  React.useEffect(() => {
    const fetchData = onSnapshot(doc(db, "users", dataId),
      (doc) => {
  ...
  React.useEffect(() => {
    async function fetchData() {
      const q = query(collection(db, "vehicles"), where("user.uid", "==", dataId));
      ...
      return ({
        ...
        action: (<>
          ...
          <MDBox display="flex" alignItems="center" mt={{ xs: 2, sm: 0 }} ml={{ xs: -1.5, sm: 0 }}>
            ...
            <MDButton variant="text" color={darkMode ? "white" : "dark"} onClick={() => userCardModalOpen(index)}>
              <Icon>edit</Icon>&nbsp;edit
            </MDButton>
          </MDBox></>),
      })
