...
const UserDisplayTable = ({ columns, collectionName, editable = true }) => {
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(collection(db, collectionName),where("uid","==",currentUser), (snapshot) => {
      ...
      <Table sx={{ minWidth: "100%" , maxWidth: 1000}} aria-label="simple table">
        <TableHead sx={{ display: "table-header-group" }}>
          <TableRow>
            ...
            {editable && <TableCell align="left">Actions</TableCell>}
          </TableRow>
        </TableHead>
        <TableBody>
          {rows.map((row, index) => (
