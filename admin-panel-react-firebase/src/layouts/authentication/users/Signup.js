...
function Signup() {
  ...
  const handleSignup = async (e) => {
    ...
    if (signupUser.userName !== '' || signupUser.brandName !== '' || signupUser.role === 'admin') {
      try {
        const user = await createUserWithEmailAndPassword(auth, signupUser.email, signupUser.password)
        navigate("/login")
        if (signupUser.role === 'admin' || signupUser.role === 'user' || signupUser.role === 'brand') {
          ...
          const updateBankData = {
            userName: signupUser.userName.toLowerCase().replace(/\s+/g, '').trim()
          }
      ...
      <Card>
        ...
        <MDBox pt={4} pb={3} px={3}>
          ...
          <MDBox component="form" role="form">
            ...
            {signupUser.role !== 'user' ? null : <MDBox mb={2}>
              <MDInput
